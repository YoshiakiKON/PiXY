...
From the image, posterization and connected-component analysis are used to
calculate the centroid of each color region.
"""
...
class CentroidProcessor:
    """
    Centroid calculation processor class.

    Stores the processing image and full-resolution image scale information, and
    calculates centroids based on the parameters.
    """
    ...
    def __init__(self, proc_img, scale_proc_to_full, img_full):
        ...
        Args:
            proc_img: Processing image (resized)
            scale_proc_to_full: Scale factor from processing image to full-resolution image
    ...
    def get_centroids(
        ...
        *,
        ...
        Args:
            params: Processing parameters (levels, min_area, trim_px)
            poster: Posterized image (generated internally if None)

        Returns:
            Centroid list [[group_no, cx, cy], ...]
        """
        # Generate the poster here if it was not passed in (backward compatibility)
        ...
        for group_no, color in enumerate(unique_colors, 1):
            ...
            # Trim (shrink): convert the full-image pixel value specified in the UI to proc resolution
            # and use `trim_px_proc` as the iteration count for morphological shrinking.
            if trim_px_proc > 0:
                k = int(trim_px_proc)
