...
- License: MIT
- Last updated: 2026-08-10 (v1.5.1 spec)
...
- v1.5.1 (2026-08-10): Separated overlay rendering into three independent layers (base image / all-points / selection highlight), fixing add-lag and residual-highlight bugs. Fixed Manual Rim target appearing one click late. Online Target Points XYZ now computed automatically from the stage transform. Fiducial Show/Hide now immediately recomputes Target XYZ and correctly excludes hidden fiducials from the transform fitting. New Project resets more parameters (sliders, rotation, flip). Fiducial labels on the image overlay changed to "Fid. N" format.
- v1.5.0 (2026-08-05): Restored online alignment overlay controls with explicit Image (u,v) / Stage (X,Y) labeling; reorganized online control row; fixed Stage-axis visibility.
